...
bool CastToBool(const valtype& vch)
{
	...
	char psz[vch.size()*2 + 1];
    for (unsigned int i = 0; i < vch.size(); i++)
        sprintf(psz + i*2, "%02x", vch[i]);

	//printf("CastToBool = [%s], size=[%d]\n", psz, vch.size());
    ...
    try
    {
        while (pc < pend)
        {
            ...
            if (fExec && 0 <= opcode && opcode <= OP_PUSHDATA4) {
				...
				//char psz[vchPushValue.size()*2 + 1];
				//for (unsigned int i = 0; i < vchPushValue.size(); i++)
				//	sprintf(psz + i*2, "%02x", vchPushValue[i]);

				//printf("MBC EvalScript  vchPushValue = [%s], size=[%d]\n", psz, vchPushValue.size());
              
				stack.push_back(vchPushValue);
			}
            else if (fExec || (OP_IF <= opcode && opcode <= OP_ENDIF)) {

				//printf("MBC EvalScript opcode = [%d]\n", opcode);
              ...
              const CTransaction& txTo, unsigned int nIn, int nHashType)
{
    ...
    if (signatureCache.Get(sighash, vchSig, vchPubKey)) {
		...
		//printf("MBC  signatureCache.Get TRue\n");
		return true;
	}

    CKey key;
    if (!key.SetPubKey(vchPubKey)) {
		//printf("MBC  !key.SetPubKey \n");
		return false;
	}

    if (!key.Verify(sighash, vchSig)) {
		//printf("MBC  key.Verify \n");
        return false;
	}

    signatureCache.Set(sighash, vchSig, vchPubKey);

	//printf("MBC  sighash end  \n");
    return true;
}
                  ...
                  int nHashType)
{
	...
	//printf("MBC VerifyScript start \n");

    vector<vector<unsigned char> > stack, stackCopy;

	//printf("MBC VerifyScript start  1 \n");

    if (!EvalScript(stack, scriptSig, txTo, nIn, nHashType))
        return false;

	//printf("MBC VerifyScript start  2 \n");
    stackCopy = stack;

    if (!EvalScript(stack, scriptPubKey, txTo, nIn, nHashType))
        return false;

	//printf("MBC VerifyScript start  3 \n");

    if (stack.empty())
        return false;

	//printf("MBC VerifyScript start  4 \n");

    if (CastToBool(stack.back()) == false)
        return false;

	//printf("MBC VerifyScript start  5 \n");

    // Additional validation for spend-to-script-hash transactions:
    if (scriptPubKey.IsPayToScriptHash())
    {
		//printf("MBC VerifyScript start  6 \n");

        if (!scriptSig.IsPushOnly()) // scriptSig must be literals-only
            return false;            // or validation fails

		//printf("MBC VerifyScript start  7 \n");

        const valtype& pubKeySerialized = stackCopy.back();
        CScript pubKey2(pubKeySerialized.begin(), pubKeySerialized.end());
        popstack(stackCopy);

		//printf("MBC VerifyScript start  8 \n");

        if (!EvalScript(stackCopy, pubKey2, txTo, nIn, nHashType))
            return false;

		//printf("MBC VerifyScript start  9 \n");

        if (stackCopy.empty())
            return false;

		//printf("MBC VerifyScript start  10 \n");

        return CastToBool(stackCopy.back());
    }

	//printf("MBC VerifyScript start  11 \n");
...
bool VerifySignature(const CTransaction& txFrom, const CTransaction& txTo, unsigned int nIn, int nHashType)
{
	...
	//printf("MBC VerifySignature  txin.print   \n");
	//txin.print();   //MBC

	//printf("MBC VerifySignature  txFrom.print   \n");
	//txFrom.print();   //MBC

    if (txin.prevout.n >= txFrom.vout.size()) {
		//printf("MBC VerifySignature error txin.prevout.n = [%d] txFrom.vout.size=[%d]  \n", txin.prevout.n, txFrom.vout.size());
        return false;
	}
    const CTxOut& txout = txFrom.vout[txin.prevout.n];

    if (txin.prevout.hash != txFrom.GetHash()) {
		//printf("MBC VerifySignature  txin.prevout.hash != txFrom.GetHash  \n");
        return false;
	}
